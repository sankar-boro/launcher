...
export async function para_run(config_dir: string, rawConfig: LaunchConfig) {
	...
	for (const parachain of config.parachains) {
		...
		if (chain) {
			const paraFullChainName = chain;
			const chainSpec = `${paraFullChainName}.para.json`;
			const rawChainSpec = `${paraFullChainName}.para.raw.json`;
			...
			if ((!config.reuseChainSpec && chainSpecExists) || !chainSpecExists) {
				...
				chain = resolve(`${paraFullChainName}.para.json`);
				...
				console.log(`\`reuseChainSpec\` flag enabled, will use existing \`${paraFullChainName}.para.raw.json\`, delete it if you don't want to reuse`);
				chain = resolve(`${paraFullChainName}.para.raw.json`);
			}
